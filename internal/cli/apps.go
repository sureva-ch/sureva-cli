...
--wait blocks until the domain becomes active (useful for CI pipelines).`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			useExistingRepo, _ := cmd.Flags().GetBool("use-existing-repo")
			...
			if err != nil {
				// The repository name is taken. Without this hint the caller sees
				// only a conflict and no way forward, which is exactly the dead
				// end the flag exists to open. Keyed on the server's own code
				// because an app-name clash returns 409 as well, and pointing
				// that one at --use-existing-repo would be wrong advice.
				var apiErr *client.APIError
				if !useExistingRepo && errors.As(err, &apiErr) && apiErr.ServerCode == "github_repo_exists" {
					apiErr.Message += " — re-run with --use-existing-repo to create the app from it instead (its contents are left untouched), or choose another --name"
				}
				return handleAPIError(r, err)
			}
