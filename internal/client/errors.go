...
type APIError struct {
	...
	Code       string
	Message    string
	// ServerCode is the `code` field cloud-api attached to the response, when it
	// sent one. Code above is derived from the HTTP status and is the CLI's
	// stable, script-facing vocabulary, so it must not change meaning; several
