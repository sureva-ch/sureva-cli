...
- Go toolchain moved to `go1.26.6`. `govulncheck` reported six standard-library
  vulnerabilities reachable from `internal/authflow` — HTTP serving, the OAuth
  code exchange, and listener teardown. Pre-existing and time-based: `main`
  reported the same six, because the scan runs against a live vulnerability
  database and the last green run predates their disclosure. Now zero.

### Added

- `apps create --use-existing-repo` creates the app from an existing GitHub
  repository of the same name instead of failing on the conflict. The
  repository's contents are left untouched.
- Without the flag, that conflict now names the way out instead of just
  reporting the clash. The hint is keyed on the server's `code`, since an
  app-name clash also answers `409` and pointing that one at
  `--use-existing-repo` would be wrong advice.
